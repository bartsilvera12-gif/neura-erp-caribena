-- =============================================================================
-- Módulo CAJA por turno — SOLO schema `caribenaerp` (instancia Caribeña).
--
...
--
-- Crea:
--   · caribenaerp.cajas             (un turno de caja: apertura/cierre/arqueo)
--   · caribenaerp.caja_movimientos  (ingresos/egresos/retiros/ajustes manuales)
--   · caribenaerp.ventas.caja_id    (asocia cada venta nueva a su caja)
--
-- Reglas:
...
--     RLS/trigger viven DENTRO del schema caribenaerp. Todas las referencias son
--     schema-local (caribenaerp.*). NO toca public, zentra_erp ni otros clientes.
--   · NO reasigna ventas históricas: caja_id queda NULL en las previas.
--   · empresa_id → caribenaerp.empresas; usuarios → caribenaerp.usuarios
--     (mismo patrón que caribenaerp.ventas / caribenaerp.proyectos).
--   · RLS: caribenaerp.puede_acceder_empresa(empresa_id). Trigger: caribenaerp.set_updated_at.
-- =============================================================================

DO $$
DECLARE
  sch text := 'caribenaerp';
BEGIN
