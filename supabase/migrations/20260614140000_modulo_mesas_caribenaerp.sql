-- =============================================================================
-- Módulo MESAS (pedidos por tablet de mozo) — SOLO schema `caribenaerp`.
--
...
-- modulos) y funciones RLS/trigger viven DENTRO de caribenaerp. Todo schema-local.
...
INSERT INTO caribenaerp.mesas (empresa_id, numero)
SELECT e.id, g.n
FROM caribenaerp.empresas e
CROSS JOIN generate_series(1, 12) AS g(n)
WHERE NOT EXISTS (
  SELECT 1 FROM caribenaerp.mesas m WHERE m.empresa_id = e.id AND m.numero = g.n
);

-- Catálogo de módulos: alta de "mesas" (schema-local) + activación para la empresa.
INSERT INTO caribenaerp.modulos (nombre, slug, descripcion)
SELECT 'Mesas', 'mesas', 'Pedidos por mesa para mozos (tablet)'
WHERE NOT EXISTS (SELECT 1 FROM caribenaerp.modulos WHERE slug = 'mesas');
