-- =============================================================================
-- Inventario gastronómico — campos aditivos en productos (caribenaerp)
-- Idempotente. Solo schema caribenaerp. NO toca otros schemas.
-- =============================================================================
...
COMMENT ON COLUMN caribenaerp.productos.controla_stock IS
  'Si false, el producto no descuenta stock (ajustes, servicios, tarifas).';
COMMENT ON COLUMN caribenaerp.productos.valorizado IS
  'Si false, no entra en valuación de inventario (combos, promociones).';
COMMENT ON COLUMN caribenaerp.productos.unidad_compra IS
  'Unidad usada al comprar (ej. "Bolsa 25kg").';
COMMENT ON COLUMN caribenaerp.productos.unidad_receta IS
  'Unidad usada en recetas (ej. "g", "ml").';
COMMENT ON COLUMN caribenaerp.productos.factor_compra_receta IS
  'Factor para convertir 1 unidad de compra a unidades de receta (ej. 25000 g por bolsa).';
COMMENT ON COLUMN caribenaerp.productos.tiempo_prep_minutos IS
