-- =============================================================================
-- Pedidos (Fase 1) — Solo schema caribenaerp, solo empresa Caribeña.
...
BEGIN
  ...
  SELECT id INTO v_modulo_proyectos_id FROM caribenaerp.modulos WHERE slug = 'proyectos';
  IF v_modulo_proyectos_id IS NULL THEN
    RAISE EXCEPTION 'Slug proyectos no existe en caribenaerp.modulos';
  END IF;
  ...
  UPDATE caribenaerp.proyecto_estados
  SET activo = false
