# Smoke test — Módulo Caja por turno (caribenaerp)

Valida la migración `supabase/migrations/20260614120000_modulo_caja_turno_caribenaerp.sql`
y el flujo completo de caja (abrir / mover / vender / cerrar / reportes).

Script: `scripts/smoke-caja-caribenaerp.ts` · Comando: `npm run smoke:caja`
...
| `SMOKE_EMAIL` / `SMOKE_PASSWORD` | usuario de la empresa caribenaerp (para el JWT) |
...
```bash
SMOKE_EMAIL=usuario@caribenaerp SMOKE_PASSWORD=*** npm run smoke:caja
```
...
SMOKE_EMAIL=usuario@caribenaerp SMOKE_PASSWORD=*** \
npm run smoke:caja
```

### Producción (Coolify / caribenaerp.neura.com.py)
...
```bash
SMOKE_BASE_URL=https://caribenaerp.neura.com.py \
SMOKE_EMAIL=usuario@caribenaerp SMOKE_PASSWORD=*** \
npm run smoke:caja
```
